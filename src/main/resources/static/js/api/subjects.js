...
function findAll(uri) {
    $.ajax({
        ...
        success: function (data) {
            ...
            for (let i = 0; i < data.length; i++) {
                $('#subject-content').append(
                    `
                        <tr>
                            <td>${i + 1}</td>
                            <td> ${data[i].name } ${data[i].grade.name} </td>
                            <td>
...
$('#next-page').on('click', function () {
    if (!$('#next-page').hasClass('disabled')) {
        ...
        findAll(url + '/api/v1/subjects?page=' + page + '&size=' + size + '&sort=grade');
    }
...
$('#add-modal-form').submit(function (e) {
    ...
    $.ajax({
        ...
        statusCode: {
            ...
            400: function () {
                $('#add-modal').modal('hide');
                $('#notification-message').text("Môn học này đã tồn tại!");
                $('#notification-modal').modal('show');
            }
        }
...
function updateModalContent(id) {
    ...
    $.ajax({
        url: url + '/api/v1/subjects/' + id,
        method: 'GET',
        contentType: 'application/json',
        success: function (data) {
            $('#update-name').val(data.name);
            $("#update-grade").val(data.grade.id).change();
            $('#update-subject').val(id);
        }
    })
}
$('#update-modal-form').submit(function (e) {
    e.preventDefault();
    $.ajax({
        url: url + '/api/v1/subjects/' + $('#update-subject').val(),
        method: 'PUT',
        contentType: 'application/json',
        data: JSON.stringify({
        ...
        statusCode: {
            ...
            400: function () {
                $('#update-modal').modal('hide');
                $('#notification-message').text("Môn học này đã tồn tại! Vui lòng thay đổi thông tin khác!");
                $('#notification-modal').modal('show');
            }
        }
...
$('#delete-modal-form').submit(function (e) {
    e.preventDefault();
    $.ajax({
        url: url + '/api/v1/subjects/' + $('#delete-subject').val(),
        method: 'DELETE',
        contentType: 'application/json',
        statusCode: {
            204: function () {
